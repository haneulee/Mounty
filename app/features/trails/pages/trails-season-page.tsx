...
export const meta: Route.MetaFunction = () => {
  return [
    { title: "Trails by Season - Mounty" },
    { name: "description", content: "Find trails by season" },
  ];
};

export default function TrailsSeasonPage({ loaderData }: Route.ComponentProps) {
  return (
    <main className="container mx-auto px-4 py-8">
      <h1 className="text-4xl font-bold mb-8">Trails by Season</h1>
        ...
        {loaderData.trails.map((trail: Trail) => (
          <div key={trail.id} className="p-4 border rounded-lg">
            ...
            <div className="mt-4 flex items-center gap-4 text-sm text-muted-foreground">
              ...
              <span>·</span>
              <span className="capitalize">{trail.season}</span>
            </div>
          </div>
        ))}
      </div>
